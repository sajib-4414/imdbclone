...
- **story: so many user type solutions**: I also used a proxy model, RegularUser, StaffUser, this allows to set the role automatically if you create user with theese proxy models, these are just a virtual copy of regular user model i createad as a custom. So i created different proxy users, with auto set some properties, such as staff admin user, main admin user, and some of the field values. so the code was more manageable. for example, admin has access to admin panel user settings yes, but the staff level1 doesnot have the access to user settings, No. staff level2 has access to notifications. so this kind of Role based Access Controls.
